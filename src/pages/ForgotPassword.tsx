          ...
          <div className="rounded-b-2xl border border-t-0 border-border bg-card p-6 shadow-card">
            {sent ? (
              <div className="text-center space-y-4 py-4">
                ...
                <p className="font-cairo text-sm text-muted-foreground">تم إرسال رابط استعادة كلمة المرور إلى بريدك الالكتروني</p>
                <Link to="/sign-in">
                  <Button variant="outline" className="font-cairo">العودة لتسجيل الدخول</Button>
                </Link>
              </div>
            ) : (
              <>
                <p className="mb-4 text-center font-cairo text-sm text-muted-foreground">ادخل البريد الالكتروني لاسترجاع كلمة المرور الخاصة بك</p>
                <form onSubmit={handleSubmit} className="space-y-4">
                  <Input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="example@domain.com" className="text-left" dir="ltr" maxLength={255} />
                ...
                <div className="mt-4 text-center">
                  <Link to="/sign-in" className="font-cairo text-sm text-primary hover:underline">او العودة الى صفحة تسجيل الدخول</Link>
                </div>
              </>
            )}
          </div>
